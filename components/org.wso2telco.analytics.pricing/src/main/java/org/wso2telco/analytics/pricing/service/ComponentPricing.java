...
public class ComponentPricing {
    ...
    protected static void priceComponent(ChargeRate rate, Map.Entry<CategoryCharge, BilledCharge> categoryEntry, List<Tax> taxList, StreamRequestData reqdata) throws AnalyticsPricingException {
        ...
        BigDecimal billRate = rate.getTarrif().getValue();
        ...
        switch (rate.getType()) {
            ...
            case CONSTANT:
                ...
                if (SubsRate != null) {
                    billRate = ((Tariff) SubsRate).getValue();
                }
            ...
            case QUOTA:
                ...
                int maxCount = rateAttributes.getTariffmaxcount();
                BigDecimal excessRate = (rateAttributes.getExcessRate());
                BigDecimal defaultRate = (rateAttributes.getDefaultRate());
...
//                break;
    ...
    private static void applyChargesForPaymentApi(ChargeRate chargeRate, Map.Entry<CategoryCharge, BilledCharge> categoryEntry, StreamRequestData reqdata,
        ...
        if (CategoryBased) {
            ...
            if (SubsRate != null) {
                Tariff commisionRates = (Tariff) SubsRate;
                ...
                adscomPercnt = rate.getTarrif().getAdsCommission().divide(new BigDecimal(100));
                spcomPercnt = rate.getTarrif().getSpCommission().divide(new BigDecimal(100));
                opcomPercnt = rate.getTarrif().getOpcoCommission().divide(new BigDecimal(100));
            }

            // Merchant based charging - to be implemented
...
//        }
    ...
    private static void applyChargesWithTax(Map.Entry<CategoryCharge, BilledCharge> CatEntry, ChargeRate rate, StreamRequestData reqdata,
        ...
        Tariff tariff = rate.getTarrif();
        BigDecimal billRate = tariff.getValue();
